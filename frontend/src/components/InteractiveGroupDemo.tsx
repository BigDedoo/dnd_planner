                ...
                <div className="p-3 sm:p-5">
                    ...
                    <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1fr)_250px]">
                        <DemoSurface className="p-4 sm:p-5" active={feature === "calendar"} setSurfaceRef={setSurfaceRef("calendar")}>
                            <div className="flex flex-col gap-3">
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="font-serif text-lg font-bold text-stone-100">Group Calendar</h3>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-1 self-start rounded-lg border border-slate-700 bg-[#141c26] p-1 text-xs text-slate-300 sm:self-auto">
                                        <MonthButton label="Previous year" onClick={() => setDisplayedMonth((current) => subMonths(current, 12))}>«</MonthButton>
                                        <MonthButton label="Previous month" onClick={() => setDisplayedMonth((current) => subMonths(current, 1))}><ChevronLeft size={14} /></MonthButton>
                                        <span className="min-w-28 border-x border-slate-700 px-2.5 py-1 text-center font-semibold">{format(displayedMonth, "MMMM yyyy")}</span>
                                        <MonthButton label="Next month" onClick={() => setDisplayedMonth((current) => addMonths(current, 1))}><ChevronRight size={14} /></MonthButton>
                                        <MonthButton label="Next year" onClick={() => setDisplayedMonth((current) => addMonths(current, 12))}>»</MonthButton>
                                        <button type="button" onClick={() => { const today = new Date(); setDisplayedMonth(today); setSelectedDate(today); }} className="rounded-md px-2 py-1 text-[10px] font-bold text-amber-100 transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-200/70">Today</button>
                                    </div>
                                </div>
                                <p className="text-[10px] text-slate-500">Select a date to inspect it. Use its small status control to update your answer.</p>
                                <div aria-label="Your availability legend" className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-slate-400">
                            ...
                            <div className="mt-4 grid grid-cols-7 gap-1.5 sm:gap-2">{weekdays.map((day) => <div key={day} className="pb-1 text-center text-[9px] font-bold uppercase tracking-wide text-slate-500">{day}</div>)}{startPadding.map((_, index) => <div key={`blank-${index}`} className="min-h-[68px] rounded-md border border-transparent bg-[#141c26]/40 sm:min-h-[92px]" />)}{daysInMonth.map((date) => { const dateKey = format(date, "yyyy-MM-dd"); return <CalendarDay key={dateKey} date={date} selected={isSameDay(date, selectedDate)} scheduledTime={sessionsByDay[dateKey]?.startTime} summary={getDaySummary(dateKey, availabilityByDay)} youAvailability={availabilityByDay[dateKey]?.Daerrus ?? null} availabilityHint={surfaceProps("calendar")} describedBy={describedBy("calendar")} onSelect={() => selectDate(date)} onCycleAvailability={() => cycleMyAvailability(date)} />; })}</div>
                        </DemoSurface>
...
function CalendarDay({ date, selected, scheduledTime, summary, youAvailability, availabilityHint, describedBy, onSelect, onCycleAvailability }: { date: Date; selected: boolean; scheduledTime?: string; summary: DaySummary | null; youAvailability: DemoAvailability; availabilityHint: DemoSurfaceProps; describedBy?: string; onSelect: () => void; onCycleAvailability: () => void }) {
    const currentDay = isToday(date);
    const dateLabel = format(date, "MMMM d");
    const fillStyle = availabilityFillRateStyle(summary?.availableCount ?? 0, demoPlayers.length);
    return <div role="button" tabIndex={0} aria-label={`Select ${dateLabel}`} onClick={onSelect} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(); } }} className={`relative flex min-h-[68px] cursor-pointer flex-col justify-between rounded-md border p-0.5 text-left transition focus:outline-none focus:ring-2 focus:ring-amber-200/70 sm:min-h-[92px] sm:p-2 ${selected ? "border-amber-200 bg-amber-200/[0.09] ring-1 ring-amber-200/70" : scheduledTime ? "border-amber-300/55 bg-amber-200/[0.05] hover:border-amber-200" : "border-slate-700/80 bg-[#151d27] hover:border-slate-600"} ${currentDay ? "font-bold shadow-[inset_0_0_0_1px_rgba(96,165,250,0.35)]" : ""}`}><div className="flex flex-col items-center gap-0.5 sm:flex-row sm:justify-between"><span className={`flex size-5 items-center justify-center rounded-full text-xs font-bold ${currentDay ? "bg-sky-400 text-slate-950" : "text-slate-300"}`}>{format(date, "d")}</span><button type="button" aria-describedby={describedBy} onPointerEnter={availabilityHint.onSurfaceEnter} onPointerLeave={availabilityHint.onSurfaceLeave} onFocus={availabilityHint.onSurfaceFocus} onBlur={() => availabilityHint.onSurfaceLeave?.()} aria-label={`Your availability for ${dateLabel}: ${availabilityName(youAvailability)}. Set ${nextAvailabilityLabel(youAvailability)}`} title={`Cycle availability: ${nextAvailabilityLabel(youAvailability)}`} onClick={(event) => { event.stopPropagation(); onCycleAvailability(); }} onKeyDown={(event) => event.stopPropagation()} className={`inline-flex min-h-7 w-full min-w-6 items-center justify-center rounded border border-current/30 text-xs font-extrabold transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-200/70 sm:w-7 ${availabilityButtonTone(youAvailability)}`}>{availabilitySymbol(youAvailability)}</button></div><div className="mt-1 space-y-1">{scheduledTime && <div className="rounded bg-amber-200/14 px-1.5 py-0.5 text-[8px] font-bold text-amber-100 sm:text-[9px]">✓ {scheduledTime}</div>}{summary && (summary.availableCount > 0 ? <div className={`flex items-center gap-1 text-[9px] font-semibold sm:text-[10px] ${fillStyle.text}`}><span aria-hidden="true" className={`size-2 shrink-0 rounded-full ${fillStyle.dot}`} />{summary.availableCount}/{demoPlayers.length}</div> : summary.maybeCount > 0 ? <div className="flex items-center gap-1 text-[9px] font-semibold text-amber-200 sm:text-[10px]"><span aria-hidden="true">🟡</span>{summary.maybeCount} maybe</div> : summary.noCount > 0 ? <div className="text-[9px] font-semibold text-slate-500 sm:text-[10px]">{summary.noCount} no</div> : null)}</div></div>;
}
...
function availabilityName(status: DemoAvailability) { return status === "No" ? "Unavailable" : status ?? "No response"; }
...
function availabilityButtonTone(status: DemoAvailability) { return status === "Available" ? "bg-emerald-400/15 text-emerald-200" : status === "Maybe" ? "bg-amber-300/15 text-amber-100" : status === "No" ? "bg-rose-400/15 text-rose-200" : "bg-slate-800/70 text-slate-300 hover:text-amber-100"; }

...
type DemoFeature = "best-dates" | "calendar" | "schedule" | "rsvp";
...
const explanations: Record<DemoFeature, { title: string; copy: string; detail: string }> = {
    ...
    calendar: { title: "Shared Calendar", copy: "Inspect a date without changing it, then use the small status control to update your answer.", detail: "The control cycles Available, Maybe, No, then Clear." },
    ...
    rsvp: { title: "RSVP & My Schedule", copy: "Availability says you could play; RSVP says you are going.", detail: "Confirmed sessions then have a clear place in My Schedule." },
};
...
export function InteractiveGroupDemo() {
    ...
    const positionPopup = useCallback((nextFeature: DemoFeature) => {
        ...
        const getCandidate = ({ side, align, offsetX, offsetY }: PopupPlacement) => {
            ...
            const top = side === "above" ? rect.top - estimatedHeight - gap + offsetY : side === "below" ? rect.bottom + gap + offsetY : align === "start" ? rect.top + offsetY : rect.bottom - estimatedHeight + offsetY;
        ...
        const fitsViewport = ({ top, left }: { top: number; left: number }) => top >= margin && left >= margin && top + estimatedHeight <= window.innerHeight - margin && left + popupWidth <= window.innerWidth - margin;
        const preferred = candidates.find(fitsViewport) ?? candidates[0];
        setPopupPosition({ mode: "anchored", top: Math.max(margin, Math.min(preferred.top, window.innerHeight - estimatedHeight - margin)), left: Math.max(margin, Math.min(preferred.left, window.innerWidth - popupWidth - margin)) });
    }, []);
    ...
    const selectDate = (date: Date, source: DemoFeature = "calendar") => {
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-600/80 bg-[#18212c] shadow-[0_28px_80px_rgba(0,0,0,0.48)]">
                ...
                <div className="p-3 sm:p-5">
                    ...
                    <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1fr)_250px]">
                        <DemoSurface className="p-4 sm:p-5" {...surfaceProps("calendar")}>
...
function DemoSurface({ active, className, children, setSurfaceRef, onSurfaceEnter, onSurfaceLeave, onSurfaceFocus, onSurfaceBlur, onSurfacePin, ...props }: DemoSurfaceProps) {
    return <div ref={setSurfaceRef} onMouseEnter={onSurfaceEnter} onMouseLeave={onSurfaceLeave} onFocus={onSurfaceFocus} onBlur={onSurfaceBlur} onClick={onSurfacePin} className={`relative rounded-xl border bg-[#1a232e] shadow-[0_12px_28px_rgba(0,0,0,0.16)] transition duration-200 ${active ? "border-amber-200/65 bg-amber-200/[0.07] shadow-[0_0_0_1px_rgba(224,182,110,0.16),0_14px_32px_rgba(0,0,0,0.22)]" : "border-slate-700/80"} ${className ?? ""}`} {...props}>{children}</div>;
}

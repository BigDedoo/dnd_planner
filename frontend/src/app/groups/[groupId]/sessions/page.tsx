    ...
    return <div className="min-h-screen bg-[#111820] text-slate-100">
        ...
        <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
            <div className="mb-7 flex flex-wrap items-end justify-between gap-3">
                <div className="min-w-0"><p className="break-words text-xs font-bold uppercase tracking-widest text-amber-200/70">{group?.name || "Group"}</p><h1 className="mt-1 font-serif text-3xl font-bold">Sessions</h1><p className="mt-2 text-sm text-slate-400">Scheduled sessions and your RSVP are separate from day availability.</p></div>
...
function SessionSection({ title, sessions, empty, groupName, groupId, canManage, updatingId, onRsvp, onExport, cancelled = false }: {
    ...
    return <section><h2 className="mb-3 font-serif text-xl font-bold text-stone-100">{title}</h2>{sessions.length === 0 ? <p className="rounded-xl border border-dashed border-slate-700 p-6 text-sm text-slate-500">{empty}</p> : <div className="grid gap-3 lg:grid-cols-2">{sessions.map((session) => {
        ...
        return <article key={session.id} className={`rounded-xl border p-4 ${cancelled ? "border-slate-700 bg-slate-900/40 opacity-70" : "border-slate-700 bg-[#18212c]"}`}>
            <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-serif text-lg font-bold text-stone-100">{session.title || "DnD session"}</p><p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-300"><CalendarDays size={13} /> {session.day}{session.start_time && <><Clock3 size={13} className="ml-1" /> {session.start_time.slice(0, 5)}</>}</p></div><span className="rounded bg-slate-800 px-2 py-1 text-[10px] font-bold uppercase text-slate-400">{session.my_rsvp ? session.my_rsvp : "No RSVP"}</span></div>
            {session.start_time && session.group_timezone && <p className="mt-1 text-[11px] text-slate-500">{session.group_timezone}</p>}

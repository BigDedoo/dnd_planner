...
def test_plan_requires_exact_head_and_writes_no_domain_rows(
    ...
    assert plan["destination"]["alembic_revision"] == "0013_group_availability_override"
...
def test_destination_revision_mismatch_is_rejected_before_domain_reads(
    ...
    try:
        ...
        with postgres_engine.begin() as connection:
            connection.execute(
                sa.text(
                    "UPDATE alembic_version SET version_num = '0013_group_availability_override'"
                )
            )
...
def test_apply_verify_and_immediate_rerun_are_exact_and_idempotent(
    ...
    # A new app-managed preference must not invalidate the legacy-field
    # checksum, and the importer must not reset an existing user's choice.
    with postgres_engine.begin() as connection:
        connection.execute(
            sa.text(
                "UPDATE users SET session_reminder_minutes = NULL "
                "WHERE display_name = 'Quentin'"
            )
        )
    preference_report = tmp_path / "preference-rerun-report.json"
    assert (
        importer.main(apply_arguments(prepared, preference_report))
        == importer.ExitCode.SUCCESS
    )
    assert (
        json.loads(preference_report.read_text(encoding="utf-8"))["transaction_outcome"]
        == "already_applied"
    )
    with postgres_engine.connect() as connection:
        assert (
            connection.scalar(
                sa.text(
                    "SELECT session_reminder_minutes FROM users "
                    "WHERE display_name = 'Quentin'"
                )
            )
            is None
